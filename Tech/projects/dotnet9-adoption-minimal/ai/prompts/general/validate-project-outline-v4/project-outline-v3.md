...
Important Risk:
- Legacy Project may have initial browser compatibility issues related to Ajax - test for this early on  (change 1)
...
- Comprehensive smoke testing - all pages for all user types - be alert for Browser incompatibility issues that may impact Ajax function  (change 2)
...
- change unit test tool to xunit  (change 3)
...
- Attempt to add unit tests that my be useful in phase 4 when the many things will be broken at once.  (change 4)
- investigate adding JavaScript Unit Test that my be useful if the client side code is broken in phase 4  (change 5)
- Add Application Insights instrumentation for local development ( change 6)
...
- phase-4e-ui-compatibility - **Focus on functional parity** - make everything work the same   (change 7)
